...
@st.cache_resource
def load_assets():
    # Attempt to locate files in multiple possible structures
    possible_paths = [
        {"model": "bank_model.pkl", "encoders": "encoders.pkl", "dataset": "Bank_dataset.csv", "report": "report.md"},
        {"model": "ML LAB TRYING/bank_model.pkl", "encoders": "ML LAB TRYING/encoders.pkl", "dataset": "ML LAB TRYING/Bank_dataset.csv", "report": "ML LAB TRYING/report.md"}
    ]
    
    last_error = ""
    for path_set in possible_paths:
        if os.path.exists(path_set["model"]):
            try:
                model = joblib.load(path_set["model"])
                encoders = joblib.load(path_set["encoders"])
                return model, encoders, path_set["dataset"], path_set["report"], None
            except Exception as e:
                last_error = f"Error reading {path_set['model']}: {str(e)}"
                
    # If not found or load failed
    if not last_error:
        last_error = "Model files not found in root or ML LAB TRYING/ folder."
        
    return None, None, None, None, last_error

model, encoders, dataset_path, report_path, load_error = load_assets()
...
with st.sidebar:
    ...
    if model is not None:
        ...
        st.markdown("**Files in workspace root:**")
        st.code(str(os.listdir('.')))
        if os.path.exists("ML LAB TRYING"):
            st.markdown("**Files in 'ML LAB TRYING':**")
            st.code(str(os.listdir('ML LAB TRYING')))
...
with tab_predict:
    ...
    if model is not None and encoders is not None:
        ...
        st.warning("Model resources are not available. Check the sidebar error message for details.")
...
with tab_dataset:
    ...
    if dataset_path and os.path.exists(dataset_path):
        ...
        st.info("Dataset file not found or could not be verified.")
